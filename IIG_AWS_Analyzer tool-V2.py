...
try:
    myappid = 'IIG.AWS.DATA.ANALYSER TOOL.V2'
...
def main():
    ...
    if not os.path.isdir(working_dir): sys.exit()
    os.chdir(working_dir)

    while True:
        # Check for binary files by extension or content
        bin_exists = any(f.lower().endswith('.tob1') for f in os.listdir('.'))
        iig_header(working_dir, bin_exists)
        
        files = [f for f in os.listdir('.') if f.endswith(('.dat', '.csv'))]
        for i, f in enumerate(files): print(f"  {i:<10} {f}")
        ...
        try:
            ...
            dtype = "DATA"
            if "min" in file_name.lower(): dtype = "MINUTELY"
            elif "hour" in file_name.lower(): dtype = "HOURLY"
            elif "daily" in file_name.lower(): dtype = "DAILY"

            try:
                header = pd.read_csv(file_name, skiprows=1, nrows=1, encoding='utf-8').columns.tolist()
                df = pd.read_csv(file_name, skiprows=4, names=header, na_values=['NAN'], low_memory=False, encoding='utf-8')
            ...
            print(f" [!] Error: {e}"); os.system('pause'); continue

        print("    [MOVING AVERAGE FILTERING ]    ")
        print("[0] Raw [1] 3-pt [2] 5-pt [3] 10-pt")
        window = {"1": 3, "2": 5, "3": 10}.get(input(" >> Select: ").strip(), 1)
        ...
        s_input = input(f" >> Start (YYYY-MM-DD): ") or str(min_d.date())
        e_input = input(f" >> End   (YYYY-MM-DD): ") or str(max_d.date())
        ...
        try:
            ...
            for i, (title, cols) in enumerate(groups.items()):
                ...
                # --- EXACT POSITIONING FOR SUBPLOT TITLE ---
            ...
            # --- DYNAMIC X-AXIS FORMATTING (HH:MM + DD-MMM) ---
            delta = plot_df.index.max() - plot_df.index.min()
            if delta.days <= 2:
                # Multi-line format: HH:MM above, DD-MMM below
            ...
            plt.savefig(f"AWS_{dtype}_{s_input}_to_{e_input}.png", dpi=300)
            plt.show()
        except Exception as e:
            print(f" [!] Plot Error: {e}"); os.system('pause')
